...
Functions:
write:                          Writes a weather data CSV-file compatible to PC-Wetterstation.
...
def write( data_folder, rain_calib_factor, last_old_rain_counter, station_name, station_height, station_type, export_data, sensor_list ):
    ...
    data_folder:                Folder where the CSV-file for PC-Wetterstation will be stored. It must be given relative to the current path.
                                The file name will be automatically "EXP_MM_YY.CSV" with MM and YY being the month an the year of the datasets.
    ...
    export_data:                Data to be written on file. The required format is a list of ordered dicts 
                                with the key being registered in sensor_list. It must contain at least the following information in this order:
    ...
    firstDate = datetime.datetime.strptime( export_data[0]['date'], '%d.%m.%Y')
...
def convertTo(read_data, last_old_rain_counter, sensor_list):
    ...
    read_data:                  List containing the new read data to be processed. For each time a list containing the data with the 
                                indices specified in 'sensor_list' must be present. The units of the data must be as follows:
    ...
    for line in export_data[:]:
        curr_time = datetime.datetime.fromtimestamp( int( line['date'] ) );
    ...
    last_dataset = export_data.pop()
    last_dataset_time = datetime.datetime.strptime( last_dataset['date'] + ' ' + last_dataset['time'], '%d.%m.%Y %H:%M') # the accuracy is minutes
